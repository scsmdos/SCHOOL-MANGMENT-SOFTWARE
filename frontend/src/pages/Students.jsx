...
const Students = () => {
  ...
  const fetchStudents = useCallback(async () => {
    ...
    try {
      ...
      const students = (res.data?.data ?? res.data ?? []).map(s => {
        // Backend transformData already returns full absolute URL for storage-proxy paths
        ...
        const studentPhoto = resolvePhoto(s.student_photo);
        const parentPhoto = resolvePhoto(s.parent_photo);
        return {
          ...s,
          ...
          village: s.village ?? '',
          district: s.district ?? '',
          avatar: studentPhoto || `https://ui-avatars.com/api/?name=${encodeURIComponent(s.student_name ?? 'S')}&size=100&background=1e293b&color=a2a9b5&bold=true`,
          parentAvatar: parentPhoto || `https://placehold.co/200x240/1e293b/a2a9b5?text=PARENT`,
        };
      });
      setAllStudents(students);

      // Build class list
      const uniqueClasses = [...new Set(students.map(s => s.className).filter(c => c !== 'N/A'))].sort();
  ...
  const filtered = allStudents.filter(s => {
    ...
    const matchSearch = !q || s.name.toLowerCase().includes(q) || s.id.toLowerCase().includes(q) || s.phone.includes(q);
    ...
    <div className="p-6 bg-[var(--bg-main)] h-[calc(100vh-56px)] overflow-hidden flex flex-col transition-colors duration-300">
      ...
      <div className="bg-[var(--bg-panel-alt)] border border-[var(--border-color)] rounded-md flex-1 flex flex-col min-h-0 shadow-sm overflow-hidden transition-colors">
        ...
        <div className="p-3 border-b border-[var(--border-color)] flex items-center justify-between shrink-0 transition-colors bg-[var(--bg-panel-alt)] rounded-t-md">
           ...
           <div className="flex items-center rounded border border-[var(--border-color)] dark:border-[#334155] overflow-hidden h-9 bg-white dark:bg-transparent shadow-sm dark:shadow-none">
              ...
              <div className="relative h-full bg-white dark:bg-[#10162A] transition-colors">
                <select 
                  value={classFilter}
                ...
                <tr key={row.rawId} className="border-b border-gray-200 dark:border-[#334155] hover:bg-white/5 dark:hover:bg-[#151c2e] transition-colors group">
                  ...
                  <td className="px-5 py-2 whitespace-nowrap">
                     <div className="flex items-center justify-end space-x-1.5">
                        <button title="Generate ID Card" className="w-6 h-6 rounded flex items-center justify-center bg-white dark:bg-[#10162A] border border-[#d946ef] text-[#d946ef] hover:bg-[#d946ef]/10 transition-colors shadow-sm"><CreditCard size={10} strokeWidth={2.5}/></button>
