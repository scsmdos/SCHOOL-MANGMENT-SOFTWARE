...
for(let i=0; i<lines.length; i++) {
    if (lines[i].includes('filteredRoutes.map(route => (') || lines[i].includes('filteredRoutes.map(route => {') ) {
        start = i;
    }
    if (start !== -1 && lines[i].includes('                </div>') && i > start + 30) {
        end = i;
        break;
    }
}
...
if (start !== -1 && end !== -1) {
    // We need to find the start of the container div that precedes the map
    let containerStart = start - 1;
    lines.splice(containerStart, end - containerStart + 1, newBlock);
